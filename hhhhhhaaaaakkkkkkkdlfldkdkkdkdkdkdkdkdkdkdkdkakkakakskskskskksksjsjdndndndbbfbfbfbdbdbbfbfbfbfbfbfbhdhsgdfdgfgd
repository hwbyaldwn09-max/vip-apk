...
PHONE_PASSWORDS = ["077", "078", "079","075"]
...
============================================================
    ...
    # توكن البوت - إدخال من المستخدم
    print("\n[+] Telegram Configuration:")
    bot_token = input("Enter your Telegram Bot Token (press Enter to skip): ").strip()
    BOT_TOKEN = bot_token
    
    if BOT_TOKEN:
        chat_ids_input = input("Enter Telegram Chat IDs (comma separated, e.g., 123,456): ").strip()
        if chat_ids_input:
    ...
    print("[+] Using 300 concurrent tasks for maximum speed\n")
